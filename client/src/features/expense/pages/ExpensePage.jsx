...
const ExpensePage = () => {
  ...
  const { pagination, expenses, error, summary, summaryLoading, summaryError ,loading,urgentDues} =
  ...
  useEffect(() => {
    ...
    if (bothDatesSelected) {
      dispatch(
        fetchExpenses({ page, search, category, sort, fromDate, toDate })
      );
  ...
  return (
    <div className="max-w-7xl mx-auto">
      ...
      <div className="sm:mt-20 flex flex-col text-center sm:flex-row flex-wrap justify-between items-stretch gap-6 mb-14">
        ...
        <div className="flex-1 min-w-[260px] bg-[rgb(249,250,255)] p-6 rounded-xl border border-gray-100 shadow-sm ">
          <p className="text-yellow-700 text-sm sm:text-base font-semibold">
            Upcoming Dues
          </p>
          <p className="text-gray-500 text-xs sm:text-sm mt-1">
            ...
            <div className="flex items-center gap-2">
              ...
              >
                <path
                  ...
                  strokeLinejoin="round"
                  strokeWidth={2}
                  d="M12 8v4l3 3m-3-9a9 9 0 110 18 9 9 0 010-18zM17 9h.01M12 12v.01"
                />
              </svg>
              Monthly Dues
            </div>
      ...
      {activeTab === "dues" && <ExpenseDueList dues={dues} scrollRef={ref} />}

      {showForm && (
        <ExpenseForm expenseId={editExpenseId} onClose={handleFormClose} />
      )}
      {showDueForm && (
        <ExpenseDueForm
